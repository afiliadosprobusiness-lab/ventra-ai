import { Building2, Plus, Users, Settings, MoreHorizontal } from "lucide-react";
import { Button } from "@/components/ui/button";

const workspaces = [
  { name: "Workspace principal", role: "Owner", members: 8, plan: "Growth", status: "Activo" },
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-xs text-muted-foreground">{w.role} · {w.plan} · {w.members} miembros</p>
                </div>
              </div>
              <div className="flex items-center gap-2">
                <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${w.status === 'Activo' ? 'bg-success/10 text-success' : 'bg-muted text-muted-foreground'}`}>{w.status}</span>
